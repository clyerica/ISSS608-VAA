...
ggplot(data=realis2018_selected, 
       aes(area = `Total Unit Sold`,
           ...
           subgroup2 = `Planning Area`)) + 
  geom_treemap() +
  geom_treemap_subgroup2_border(colour = "orange",
                                size = 2) +
  geom_treemap_subgroup_border(colour = "darkred")
```
...
The following steps explain how to install a R package which is not available in cran.
...
Next, load the *devtools* library and install the package found in github by using the codes below.

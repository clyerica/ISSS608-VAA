...
Before a thematic map can be prepared, we have to prepare a data table with year 2020 values. The data table should include the variables PA, SZ, YOUNG, ECONOMY ACTIVE, AGED, TOTAL, DEPENDENCY.
...
::: {.callout-note title="Things to learn from code chunk above"}
...
::: {.callout-note title="Things to learn from tm_polygons()"}
...
tm_shape(mpsz_pop2020) + 
  tm_polygons("DEPENDENCY", 
              fill.scale = tm_scale_intervals( style = "fisher", n = 2)) + 
  tm_borders(fill_alpha = 0.5)
```
...
tm_shape(mpsz_pop2020) + 
  tm_polygons("DEPENDENCY", 
              fill.scale = tm_scale_intervals( style = "fisher", n = 6)) + 
  tm_borders(fill_alpha = 0.5)
```
##### 10
```{r}
tm_shape(mpsz_pop2020) + 
  tm_polygons("DEPENDENCY", 
              fill.scale = tm_scale_intervals( style = "fisher", n = 10)) + 
  tm_borders(fill_alpha = 0.5)
```
##### 20
```{r}
tm_shape(mpsz_pop2020) + 
  tm_polygons("DEPENDENCY", 
              fill.scale = tm_scale_intervals( style = "fisher", n = 20)) + 
  tm_borders(fill_alpha = 0.5)
```
:::

...
1.  Importing geospatial data in rds format into R environment.
2.  Creating cartographic quality choropleth maps by using appropriate tmap functions.
3.  Creating rate map
4.  Creating percentile map
5.  Creating boxmap

##Getting Started ###Installing and loading packages The following code chunk will install and load sf, tmap and tidyverse packages into R environment.
...
tm_shape(NGA_wp) +
  tm_polygons(fill = c("wp_functional","total_wp"),
  ...
  tm_borders(lwd = 0.1,
             fill_alpha = 1) +
  tm_layout(panel.labels=c("Distribution of functional  water point by LGAs","Distribution of total  water point by LGAs"))
```
